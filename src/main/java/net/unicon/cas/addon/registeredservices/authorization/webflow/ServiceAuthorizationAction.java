...
import net.unicon.cas.addon.registeredservices.RegisteredServiceWithAttributes;
...
public class ServiceAuthorizationAction extends AbstractAction {
    ...
    protected Event doExecute(final RequestContext requestContext) throws Exception {
        final Principal principal = getAuthenticatedPrincipalFrom(WebUtils.getTicketGrantingTicketId(requestContext));
        ...
        if (registeredService == null) {
            ...
            throw new UnauthorizedServiceException(String.format("Service [%s] is not defined in the service registry", serviceId));
        }
            ...
            throw new UnauthorizedServiceException(String.format("Service [%s] is not enabled in the service registry", serviceId));
        }
    ...
    //This should be changed in CAS 4.1 - the real TGTs instances will be available in the flow scope,
    //rendering this dance with TicketRegistry unnecessary.
    private Principal getAuthenticatedPrincipalFrom(final String tgtId) {
        final TicketGrantingTicket tgt = (TicketGrantingTicket) this.ticketRegistry.getTicket(tgtId, TicketGrantingTicket.class);
        if (tgt == null) {
            return null;
        }
        return (tgt.getAuthentication() == null) ? null : tgt.getAuthentication().getPrincipal();
